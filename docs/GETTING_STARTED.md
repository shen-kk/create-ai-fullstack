...
本手册适合第一次使用模板的开发者。不要在模板源仓库里初始化业务项目，请始终克隆到一个全新目录。

## 第一步：准备环境

需要安装 Git、Node.js 22+ 和 pnpm 11.9.0。正式项目还需要 PostgreSQL；只预览页面时不需要数据库。
...
```bash
pnpm template:init -- --preset=quick
```

准备开发正式 PostgreSQL 项目，选择标准模式：

```bash
pnpm template:init -- --preset=standard
```

需要逐项选择用户端、Redis、短信、邮件和支付等能力，选择自定义模式：

```bash
pnpm template:init -- --preset=custom
```
...
| ---------------------------- | ---------------- | ----------------- | -------------------------- |
| 项目名称、命名空间、中文名称 | 必问             | 必问              | 必问                       |
| 后台和 API 端口              | 必问             | 必问              | 必问                       |
| 用户端端口                   | 默认 3002        | 默认 3002         | 必问                       |
| 是否启用用户端               | **必问**         | **必问**          | **必问**                   |
| 数据库                       | 固定内存         | 固定 PostgreSQL   | 内存/PostgreSQL 二选一     |
| PostgreSQL 连接信息          | 不问             | 分项必问          | 选择 PostgreSQL 后分项必问 |
| 对象存储能力                 | 默认启用         | 默认启用          | 可选                       |
| Redis 能力                   | 用户端启用时启用 | 默认启用          | 可选，用户端启用时自动启用 |
| 短信能力                     | 用户端启用时启用 | 用户端启用时启用  | 可选，用户端启用时自动启用 |
| 邮件能力                     | 不启用           | 默认启用          | 可选                       |
| 支付能力                     | 不启用           | 不启用            | 可选                       |
| 填写已启用服务的真实配置     | 不写数据库       | 分项询问          | PostgreSQL 模式分项询问    |
| 立即迁移并创建管理员         | 不需要           | 默认是，可选否    | PostgreSQL 模式默认是      |

三种交互模式都会明确询问“是否启用用户端”，并输出最终选择。只有自动化使用 `--defaults` 时才不提问；自动化模式需要用户端时必须追加 `--user-web`。

完整向导会依次询问：

1. 项目英文名称，例如 `customer-center`。
2. 包命名空间，例如 `@customer-center`。
3. 中文显示名称和项目简介。
4. 后台、API、用户端端口。
5. 内存预览或 PostgreSQL + Prisma。
6. PostgreSQL 主机、端口、数据库名、用户名和密码（正式模式）。
7. 初始管理员手机号和名称。
8. 是否启用用户端。
9. 对象存储平台和其他可选服务能力。
10. 是否立即填写 Redis、对象存储、短信、邮件或支付配置。
11. 是否立即校验并初始化数据库。
...
快速内存模式跳过本步骤。正式模式如果已经选择“立即初始化”并成功，也不需要重复执行。否则先确认 `.env` 的 `DATABASE_URL` 指向当前项目专用数据库，禁止误连生产库或其他项目数据库。
