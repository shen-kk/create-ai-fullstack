...
初始化会把工作区包名从 `@template/*` 改成你的命名空间，所以必须再次安装：
...
快速内存模式跳过本步骤。正式模式先确认 `.env` 的 `DATABASE_URL` 指向当前项目专用数据库，禁止误连生产库或其他项目数据库。
...
按提示输入 `YES`。该命令会生成 Prisma Client、部署已有迁移并创建初始管理员。完成后：
