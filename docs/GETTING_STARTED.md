...
4. 初始管理员手机号、姓名和可选密码；密码留空时随机生成；
5. 是否立即执行迁移和管理员种子。

项目 JWT 和配置加密密钥自动生成，只写入 Git 忽略的 `.env`。初始管理员密码可以在隐藏输入中设置，留空时随机生成，最终保存在 `.env` 的 `DEV_ADMIN_PASSWORD`，终端不回显完整值。重新初始化已有项目时，留空会保留现有密码，避免意外重置。
