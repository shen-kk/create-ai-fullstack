...
- Admin 登录页不预填固定演示账号或 `Admin@123456`；初始化管理员使用 `.env` 中的 `DEV_ADMIN_PHONE` / `DEV_ADMIN_PASSWORD`。初始化可隐藏输入并确认管理员密码，首次留空时随机生成，已有项目重新初始化时留空保留原密码；初始化结果不回显密码；登录接口成功状态为 HTTP 200。
...
- Admin 写操作反馈统一使用全局 `AppNotice`，调用方显式声明成功或错误语义；相同文案连续触发仍会生成新的提示事件。加载失败等页面状态继续保留在内容流中。
...
- 项目通过显式的 `pnpm run template:provision` 执行 Prisma Client 生成、`prisma migrate deploy` 与管理员种子；默认要求输入 `YES`，支持 `--dry-run`，且不输出数据库连接串。
...
- `pnpm run template:verify -- --full` 在无依赖、无构建产物、无本地密钥的临时副本中验证安装前后命名空间替换、workspace 链接刷新、Prisma Client 生成、测试与生产构建。
