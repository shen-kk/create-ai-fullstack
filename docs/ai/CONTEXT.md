...
- `apps/web` 已恢复为正式用户端模板并纳入默认启动、检查和构建；提供 SSR 首页、手机号验证码注册、密码/验证码登录、短信找回密码、会话恢复、个人中心、邮箱验证绑定和响应式布局。验证码按用途隔离，5 分钟过期、60 秒重发限制、5 次错误上限、摘要存储且单次消费；必须经后台启用的腾讯云短信或 TLS SMTP 真实发送，API 不回显验证码。
