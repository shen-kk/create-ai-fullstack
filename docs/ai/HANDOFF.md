...
- Web 登录与找回密码共用的认证配置请求使用绝对 HTTP 入口，避免刷新时 SSR 的相对 `/api` 请求绕过代理；配置不可用时明确报错，不再默认手机号登录。

- 认证安全修复见 ADR-0019：验证码原子消费、登录账号规范化、头像上传边界、找回密码撤销会话、后台实时权限校验与短信发送结果检查。API 与 Web 需同步发布。
...
- 模板验收复用同一个子进程执行入口；验收与数据库初始化使用 Node 标准库转换文件 URL，避免手工处理平台路径。
...
- 2026-09-17 Web 认证配置刷新修复：新增 3 项配置加载回归测试，Web 共 12 项测试通过；完整 `pnpm check`、Doctor 和加载私有环境配置后的 5 项 API E2E 通过。尚未进行浏览器刷新人工验收。

- 2026-09-10 Ponytail 前端改造：Admin 业务 API 统一请求与刷新重试，历史弹窗统一 AppDialog；Web 验证码发送复用 composable，两端补充并发刷新、退出期间刷新和重复提交保护。完整 `pnpm check` 通过，包含 Admin 17 项、Web 9 项测试及三端生产构建。浏览器连接连续超时，弹窗键盘、滚动及页面视觉验收尚未完成；相关决策见 ADR-0020。

- 2026-09-10 安全修复：完整 `pnpm check`、22 项认证专项测试及 Doctor 通过。真实 PostgreSQL 用户端完整生命周期通过，含并发验证码消费、头像字段拒绝、重置密码后 Access/Refresh Token 失效。后台 E2E 因私有配置的管理员密码与数据库不一致未通过，需更新私有配置后重跑；未重置管理员密码。
