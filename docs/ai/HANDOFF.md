...
CLI 只选择业务功能并组合代码，不收集数据库或服务密钥。它完成模板获取、应用级裁剪、依赖安装、功能检查、模板 Git 历史移除和新 Git 仓库初始化。模板维护者拉取完整仓库后也使用同一个 `pnpm run setup`，不维护专用环境文件。pnpm 11 已有同名内置 `setup` 命令，文档和 CLI 提示不得省略 `run`。
