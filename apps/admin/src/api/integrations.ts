...
export const updateServiceResource = (
  ...
  request(`/integrations/resources/${encodeURIComponent(id)}`, {
    method: 'PUT',
    body: JSON.stringify(input),
  });
export const getServiceResourceSecrets = (id: string): Promise<Record<string, string>> =>
  request(`/integrations/resources/${encodeURIComponent(id)}/secrets`);
export const deleteServiceResource = (id: string): Promise<DeleteServiceResourceResponse> =>
  request(`/integrations/resources/${encodeURIComponent(id)}`, { method: 'DELETE' });
...
export const updateMessageTemplate = (
  ...
  request(`/integrations/message-templates/${encodeURIComponent(id)}`, {
    method: 'PUT',
    body: JSON.stringify(input),
  });
export const deleteMessageTemplate = (id: string): Promise<{ id: string }> =>
  request(`/integrations/message-templates/${encodeURIComponent(id)}`, { method: 'DELETE' });
